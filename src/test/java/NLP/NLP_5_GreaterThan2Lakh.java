...
public class NLP_5_GreaterThan2Lakh extends BaseClass {
	...
	@Test(priority=1)
	public void landing_Page_NLP5_MTO_G2() throws Throwable 
	{

		Landing lp = new Landing(BaseClass.driver);

		String name=lp.FullName(ObjectReaders.readers.get_FullName());
		ListnerClass.reportLog(" Full Name Of Customer = "+name);
		...
		ListnerClass.reportLog(" AVG Monthly sale = "+avg);
		lp.Next();
		wUtil.waitForElementToBeVisible(BaseClass.driver, lp.HowOldBusinessIcon);
		String old=lp.HowOldBusiness();
		ListnerClass.reportLog(" How Old Business ? = "+old);
		String icon=lp.CurrentAccIcon();
		ListnerClass.reportLog(" Account Type = "+icon);
