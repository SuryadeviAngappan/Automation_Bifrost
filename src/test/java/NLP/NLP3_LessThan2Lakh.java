...
public class NLP3_LessThan2Lakh extends BaseClass{
	...
	public void launchURL() throws Throwable 
	{
		...
		String URL=ObjectReaders.readers.getNLP_3();
		driver.get(URL);
	...
	@Test(priority=1)
	public void landing_Page_NLP3_LT2() throws Throwable 
	{
		...
		ListnerClass.reportLog(" Name  = "+name);
		no=lp.MobileNo(jUtil.random9NoFlipkart());
		ListnerClass.reportLog(" Monile Number  = "+no);
		lp.Next();
		wUtil.scrollToParticularElement(lp.HowOldBusinessIcon);
		String busi=lp.HowOldBusiness();
		ListnerClass.reportLog(" How Old Business  = "+busi);
		String type=lp.CurrentAcc();
		ListnerClass.reportLog(" Account Type  = "+type);
		String gst=lp.GSTRegisterd();
		ListnerClass.reportLog(" GST Registerd = "+gst);
		wUtil.scrollToParticularElement(lp.AvgSaleBtn);
		String sale=lp.MonthlySale();
...
//		ListnerClass.reportLog(" Customer's Pan No Is = "+pan);
		...
		ListnerClass.reportLog(" NLP-3 < 2lakh MTO Presanction Journey Completed  , Thank You!!");
