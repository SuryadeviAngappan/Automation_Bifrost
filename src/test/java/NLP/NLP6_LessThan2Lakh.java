...
public class NLP6_LessThan2Lakh extends BaseClass{
	...
	@Test(priority=1)
	public void landing_Page_NLP5_MTO_L2() throws Throwable 
	{
		landing lp = new landing (BaseClass.driver);

		no=lp.MobileNO(jUtil.random9NoFlipkart());
		ListnerClass.reportLog("Name Of Customer Id = "+no);
		String sale=lp.AvgMonthlySale();
		ListnerClass.reportLog(" Avg Sale Of Customer Is = "+sale);
		String gst=lp.Gst();
		ListnerClass.reportLog(" Does Customer is Gst Registerd ? = "+gst);
	...
	public void OTP() throws InterruptedException, SQLException 
	{
		...
		ListnerClass.reportLog(" OTP Is = "+otp);
	...
	public void form4() 
	{
		...
		String resi=lp.ResidenceRented();
		ListnerClass.reportLog("Customer;s Residence Renterd ? = "+resi );
		String busi=lp.BusinessRented();
		ListnerClass.reportLog("Customer;s Business Renterd ? = "+busi );
		lp.Continue();
